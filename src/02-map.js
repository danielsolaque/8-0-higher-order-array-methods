...
function getSongTitles(songs) {
  let songTitles = songs.map(element => element.title)
  return songTitles
}
...
function getSongDetails(songs) {
  let arrayTitlesWArtist = songs.map(element=>`${element.title} by ${element.artist}`)
  return arrayTitlesWArtist
}
...
function getTitleAndArtist(songs) {
  ...
  const arrayObjects = songs.map(element => {
    const myObject = {}
    const title1 = element.title
    const artist1 = element.artist
    myObject[title1] = artist1
    return myObject
  })
  return arrayObjects
}
